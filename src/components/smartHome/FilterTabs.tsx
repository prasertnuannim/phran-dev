...
export function FilterTabs({ value, onChange, loading = false }: FilterTabsProps) {
  ...
  return (
    <div className="relative w-full flex justify-center items-center gap-6 mb-8 border-b border-white/10 pb-2">
      {loading && (
        <div className="absolute right-0 flex items-center gap-2 text-xs text-white/60">
      ...
      {items.map((item) => {
        ...
        return (
          <button
            ...
            disabled={loading}
            className={`
              relative pb-1 text-sm tracking-wide font-medium
